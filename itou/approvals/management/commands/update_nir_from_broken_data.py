...
class Command(BaseCommand):
    ...
    def process(self, beneficiaire: PeBeneficiaire):
        pe_approvals = PoleEmploiApproval.objects.without_nir_ntt_or_nia().filter(
            pole_emploi_id=beneficiaire.pole_emploi_id
        )
        if len(pe_approvals) > 0:
            print(len(pe_approvals))
    ...
    def handle(self, file_path, dry_run=False, **options):
        ...
        with open(file_path, newline="") as csvfile:
            ...
            not_yet_found_pe_approvals = PoleEmploiApproval.objects.without_nir_ntt_or_nia()
            not_yet_found_pe_approvals_pe_ids = set(
                [approval.pole_emploi_id for approval in not_yet_found_pe_approvals]
            )
            beneficiaires_pe_ids = set([benef.pole_emploi_id for benef in beneficiaires])

            # print(len(not_yet_found_pe_approvals))
            # print(len(beneficiaires))
            # we only want the possible pole_emploi_ids: they are both not updated in our database, and in the CSV file
            possible_matches = sorted(beneficiaires_pe_ids & not_yet_found_pe_approvals_pe_ids)

            pbar = tqdm(total=len(beneficiaires))
            for beneficiaire in beneficiaires:
                if beneficiaire.pole_emploi_id in possible_matches:
                    # approvals = PoleEmploiApproval.objects
                    # .without_nir_ntt_or_nia.filter(pole_emploi_id=beneficiaire.pole_emploi_id)
                    pass
                pbar.update(1)
            pbar.close()

            # for approval in not_yet_found_pe_approvals:
            #     for beneficiaire in beneficiaires:
            #         if approval.pole_emploi_id == beneficiaire.pole_emploi_id:
            #             matches += 1
            #     pbar.update(1)
            # pbar.close()
